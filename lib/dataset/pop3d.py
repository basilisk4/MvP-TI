...
class Pop3d (JointsDataset):
    def __init__(self, cfg, image_set, is_train, transform=None):
        ...
        size=np.array(cfg.MULTI_PERSON.SPACE_SIZE)/2
        center=cfg.MULTI_PERSON.SPACE_CENTER
        self.bound_min=np.array(center-size)
        self.bound_max=np.array(center+size)

        if self.image_set == 'train':
            self.sequence_list = [d for d in os.listdir(self.dataset_root) if d.endswith("Train")]
            self._interval = 1#sampling
            ...
            for i, file in enumerate(anno_files):
                if i % self._interval == 0:
                    with open(file) as dfile:
                        bodies = json.load(dfile)['individuals']
                    if len(bodies) == 0:
                        continue

                    for k, v in cameras.items():
                        postfix = osp.basename(file)
                        prefix = k
                        image = osp.join(seq, 'Images', prefix, postfix)
                        ...
                        all_poses_vis = []
                        for body in bodies:
                            pose3d = np.array(body['keypoints'])\
                                .reshape((-1, 3))
                            pose3d = pose3d[:self.num_joints]

                            if np.isnan(pose3d).any():
                                continue
                            
                            
                            in_volume = np.all((pose3d >= self.bound_min) & (pose3d <= self.bound_max), axis=1)
                            if np.invert(in_volume).any():
                                print("Skipped outside of volume")
                                continue
                        ...
                        if len(all_poses_3d) > 0:
                            our_cam = {}
                            our_cam['R'] = v['R']
                            our_cam['T'] = -np.dot(v['R'].T, v['t'])
                            our_cam['standard_T'] = v['t']
    ...
    def __getitem__(self, idx):
        ...
        for k in range(self.num_views):
            data= super().__getitem__(self.num_views * idx + k)
            if not len(data)==2:
               print(data)
            i, m =data 
            input.append(i)
